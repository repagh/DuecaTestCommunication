...
#define DistriTest_cxx
// include the definition of the module class
#include "DistriTest.hxx"
...
// class/module name
const char *const DistriTest::classname = "distri-test";

// initial condition/trim table
const IncoTable *DistriTest::getMyIncoTable()
{
...
const ParameterTable *DistriTest::getMyParameterTable()
{
...
// constructor
DistriTest::DistriTest(Entity *e, const char *part, const PrioritySpec &ps) :
  ...
  w_pax1(getId(), NameSet(getEntity(), getclassname<PaxAmount>(), part),
         getclassname<PaxAmount>(), "first", Channel::Events,
         Channel::OneOrMoreEntries),
  w_pax2(getId(), NameSet(getEntity(), getclassname<PaxAmount>(), part),
  ...
  do_calc(getId(), "test stuff", &cb1, ps)
{
...
// destructor
DistriTest::~DistriTest()
{
  //
}

// as an example, the setTimeSpec function
bool DistriTest::setTimeSpec(const TimeSpec &ts)
{
...
bool DistriTest::checkTiming(const std::vector<int> &i)
{
  if (i.size() == 3) {
...
// start the module
void DistriTest::startModule(const TimeSpec &time) { do_calc.switchOn(time); }

// stop the module
void DistriTest::stopModule(const TimeSpec &time) { do_calc.switchOff(time); }
...
void DistriTest::fillSnapshot(const TimeSpec &ts, Snapshot &snap,
                              bool from_trim)
{
...
// declaration) if you specified to the SimulationModule that the size of
// state snapshots is zero
void DistriTest::loadSnapshot(const TimeSpec &t, const Snapshot &snap)
{
...
// appropriate output
void DistriTest::doCalculation(const TimeSpec &ts)
{
...
void DistriTest::trimCalculation(const TimeSpec &ts, const TrimMode &mode)
{
...
static TypeCreator<DistriTest> a(DistriTest::getMyParameterTable());
